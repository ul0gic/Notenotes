...
- **Customizable everywhere.** 2/4, 3/4, 4/4, and 5/4 time signatures for now. Custom beat colors for the background visualizer. The top bar has the shared project key and scale, and the Create toolbar has a stable Layout button that becomes Pads on Scale and Keys on Piano. Both edit the same optional degree colors. Drum count still lives in Settings for now.
