...
- **Step Play** - a Pads mode for one-switch or one-key playing. Build a scale-degree sequence like `1 3 5 8` in its chip editor, then hit the large Step button, any performance key, or a MIDI note to advance through it while Notenotes records your timing. Numbers keep walking up the scale, so `8` is the next octave in a seven-note scale and `6` is the next octave in a pentatonic scale. Individual steps can also carry an alternate note that plays every other pass through the loop.
