...
- **Project key, scale, and changes** - set the shared root and scale in the top bar. The scale picker is now a searchable family picker instead of a tiny dropdown, with Western modes, pentatonic colors, Hungarian/Klezmer colors, and honest 12-TET maqam/raga-inspired approximations grouped where they belong. The top bar also has a compact **Changes** picker for degree-based progressions like The Axis, Doo-wop, ii-V-I, and 12-bar blues; it is Off by default while the glow/playback layers are still being built. Optional Piano/MIDI **Correction**: Off keeps chromatic play, Closest / Up / Down nudge out-of-key Piano and external MIDI notes into the project scale. Pads, Controller fallback, AI context, and the optional Piano/Pad degree colors all read from the same place instead of each feature guessing on its own. The old duplicate Root/Scale controls are intentionally gone from Pads and Ctrl.
...
- [ ] **Progression / chord context** - the project now has a quiet, backward-compatible degree-based progression model and a compact Changes picker in the top bar. The next step is chord-tone glow, so the app can show what is harmonically hot right now without rewriting old snippets.
